...
Normal completes one attempt, respects the chosen automatic/manual whole-exam submission setting, and reports the actual result after submission. Virtual Loop requires an explicit final-only request and complete bound review evidence before retrying. Historical/aggregate-only reviews cannot create reusable answers or authorize a retry. Full scores are compared against the observed question total; Virtual exams are not assumed to contain 50 questions.
