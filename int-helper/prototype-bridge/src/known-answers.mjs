...
export const answerKnownQuestions = async ({ read, answer, maxQuestions = 10, save = true, stillAuthorized = () => true, now = Date.now }) => {
  ...
  while (applied < maxQuestions && now() - started < 12000) {
    ...
    if (result.imagesPending) return finish("images_pending");
    ...
    if (["resync", "pacing", "verified_answer_available", "image_read_required"].includes(result.mode)) return finish(result.mode);
    if (result.selected === undefined) return finish("answer_not_confirmed");
    if (!result.selectionReused) applied++;
    if (result.historyWarning) return finish("history_warning");
    if (result.recoveryWarning) return finish("recovery_warning");
    if (result.imagesPending) return finish("images_pending");
    if (result.navigationPending) return finish("navigation_pending");
  }
