...
export const createVirtualSchoolAdapter = ({ document, location }) => {
  ...
  const virtualReviewImage = (root, selector) => {
    ...
    return { image: source, unavailable: images.filter((candidate) => virtualImageHidden(candidate) || !virtualImageSource(candidate)).length,
      pending: source && image.complete === false ? 1 : 0, failed };
  };
  ...
  const readVirtualReview = () => {
    ...
    if (all) {
      ...
      return { ...base, verifiedReviews: reviews, reviewedQuestionNumbers: numbers, pendingImageCount, failedImageCount,
        ready: complete, reviewComplete: complete };
    }
