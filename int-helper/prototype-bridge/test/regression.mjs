...
{
  const sockets = [];
  let activeTab = 1, version = "0.15.3", handler, popupListener, delayHook, held;
  const sent = [];
  ...
  const chrome = {
    ...
    runtime: { getManifest: () => ({ version: "0.15.3" }), getURL: (p) => `chrome-extension://test/${p}`, onMessage: { addListener: (fn) => { popupListener = fn; } } },
    tabs: {
