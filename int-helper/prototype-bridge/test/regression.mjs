...
{
  const sockets = [];
  let activeTab = 1, version = "0.16.1", handler, popupListener, delayHook, held;
  const sent = [];
  ...
  const chrome = {
    ...
    runtime: { getManifest: () => ({ version: "0.16.1" }), getURL: (p) => `chrome-extension://test/${p}`, onMessage: { addListener: (fn) => { popupListener = fn; } } },
    tabs: {
  ...
  assert.equal((await handler("submit_current_exam", { examCode: "TIMED" }, 17373)).action, "confirmed");
  await assert.rejects(handler("set_current_exam_scope", { examCode: "SINGLE" }, 17373), /existing final\/Loop scope/);
  // Current-exam tests use a separate connection, preserving the final attempt.
  const boundExam = await handler("set_current_exam_scope", { examCode: "SINGLE" }, 17374);
  assert.equal(boundExam.scope.submissionAllowed, false);
  await assert.rejects(handler("set_exam_pacing", { durationMinutes: 60 }, 17374), /final-only/);
  await assert.rejects(handler("set_exam_loop", { enabled: true }, 17374), /final-only/);
  const countBefore = sent.length;
  for (const action of ["submit_current_exam", "advance_subject", "complete_current_lesson"]) {
    await assert.rejects(handler(action, { examCode: "SINGLE" }, 17374), /answers only/);
  }
  assert.equal(sent.length, countBefore);
  const submitScope = await handler("set_current_exam_scope", { examCode: "SINGLE", allowSubmit: true }, 17374);
  assert.equal(submitScope.scope.submissionAllowed, true);
  readResult = () => ({ examCode: "SINGLE", choices: [] });
  await handler("submit_current_exam", { examCode: "SINGLE" }, 17374);
  assert.equal(sent.at(-1).action, "submit_exam");
  await assert.rejects(handler("advance_subject", {}, 17374), /answers only/);
  pageInfo.path = "/student/virtual_school/exam.php";
  await assert.rejects(handler("set_scope", { subjectCode: "q294", mode: "chapter", chapter: 1 }, 17374), /course overview/);
}
