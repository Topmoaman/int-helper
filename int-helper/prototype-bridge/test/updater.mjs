...
assert.equal(isNewer('0.21.0', '0.18.0'), true);
...
const release = { tag_name: 'v0.21.0', draft: false, prerelease: false, assets: [{ name: ASSET_NAME, size: 100, browser_download_url: 'https://github.com/' + REPOSITORY + '/releases/download/v0.21.0/' + ASSET_NAME }] };
assert.equal((await latestRelease(async () => Response.json(release))).version, '0.21.0');
...
try {
  ...
  await assert.rejects(updater.apply(renameVersion('0.21.0'), { ensureIdle: async () => { throw Error('active exam'); } }), /active exam/);
  assert.equal((await updater.status()).version, payload.version);
  ...
  await assert.rejects(failed.apply(renameVersion('0.21.0')), /simulated/);
  assert.equal((await updater.status()).version, payload.version);
  ...
  const network = createUpdater({ root, runner: async () => {}, fetcher: async url => Response.json(url.includes('/releases/latest') ? release : renameVersion('0.21.0')) });
  let idleChecks = 0;
  ...
  assert.equal((await updater.status()).version, '0.21.0');
  ...
  await assert.rejects(updater.apply(renameVersion('0.22.0')), /Another update/);
  await fs.unlink(join(root, 'update.lock'));
  await fs.writeFile(join(root, 'current/README.md'), 'local edits');
  await assert.rejects(updater.apply(renameVersion('0.22.0')), /local changes/);
...
const socket = { send: text => { const message = JSON.parse(text); sent.push(message); queueMicrotask(() => service.receive(1, { type: 'update_response', id: message.id, ok: true, result: { updated: true, version: '0.21.0' } })); } };
let offline = false;
...
assert.equal(stored.helperUpdate.installed, '0.21.0');
