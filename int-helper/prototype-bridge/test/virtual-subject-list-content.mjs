...
const chrome = {
  ...
  runtime: { getManifest: () => ({ version: "0.16.1" }), getURL: value => `chrome-extension://test/${value}`, onMessage: { addListener() {} } },
  tabs: {
    query: async () => [{ id: 9, active: true, url: location.href }],
    sendMessage: async (id, message) => {
      sent.push({ id, ...message });
      if (message.action === "page_version") return { ok: true, result: { contentVersion: "0.16.1" } };
      let response;
