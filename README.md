...
**เวอร์ชัน GitHub 0.20.1** · ส่วนขยาย Chrome **0.15.1** · รวมการแก้จากรุ่นพัฒนา **0.18.5** · [ดาวน์โหลดรุ่นล่าสุด](https://github.com/Topmoaman/int-helper/releases/latest)
...
| --- | --- | --- |
| **0.20.1 (ปัจจุบัน)** | **0.18.5** | ปิดบริดจ์และคืนพอร์ตเมื่อ Codex/MCP ตัดการเชื่อมต่อ |
...
ดู [ROADMAP](ROADMAP.md) และ [ประกาศไลบรารีที่ใช้](THIRD_PARTY_NOTICES.md) ก่อนพัฒนาต่อ สำหรับออก Release ให้อัปเดตตารางเทียบรุ่นด้านบน แล้วปรับเวอร์ชัน plugin, `extension/updates.js`, manifest/content และ test expectations ให้ตรงกัน แล้ว build จาก source ก่อนสร้างไฟล์อัปเดต:

```sh
node scripts/build-release.mjs release
```

แนบ `release/int-helper-update.json` ใน stable Release ที่ tag ตรงกับเวอร์ชัน plugin เช่น `v0.20.1` ระบบตรวจ checksum และเส้นทางไฟล์ที่อนุญาต ข้อมูลจับคู่ส่วนขยายและประวัติไม่อยู่ในแพ็กเกจ ความเชื่อถือผู้เผยแพร่อาศัย GitHub HTTPS/repository ownership ไม่ใช่ลายเซ็นแยก
